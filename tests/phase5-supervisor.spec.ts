...
const workspace = { team: [{ id: "tel-1", displayName: "TEST Telesales", queuedCalls: 3, completedCalls: 8 }], checkpoints: [{ checkpoint: "morning", evidence: "TEST morning readiness evidence", readinessState: "attention", at: "2026-08-22T08:00:00Z" }, { checkpoint: "midday", evidence: "TEST midday evidence", readinessState: "risk", at: "2026-08-22T12:00:00Z" }, { checkpoint: "end_of_day", evidence: "TEST end day evidence", readinessState: "attention", at: "2026-08-22T16:00:00Z" }], exceptions: [{ id: "x-open", kind: "overdue", severity: "urgent", summary: "TEST open exception", evidence: "TEST source evidence", requiredNextAction: "TEST required follow-up", status: "open", version: 1, sourceType: "telesales_call", sourceId: "call-1", createdAt: "2026-08-29T08:00:00Z", employeeName: "TEST Telesales", customerName: "TEST Customer", operationallyOpen: true }, { id: "x-actioned", kind: "quality", severity: "watch", summary: "TEST actioned exception", evidence: "TEST actioned source", requiredNextAction: "TEST coaching", status: "actioned", version: 2, sourceType: "telesales_call", sourceId: "call-1", createdAt: "2026-08-29T09:00:00Z", employeeName: "TEST Telesales", priorAction: "escalate", operationallyOpen: true }, { id: "x-resolved", kind: "resolved", severity: "normal", summary: "TEST resolved exception", evidence: "TEST resolved evidence", requiredNextAction: "None", status: "resolved", version: 3, sourceType: "telesales_call", sourceId: "call-1", createdAt: "2026-08-29T10:00:00Z", employeeName: "TEST Telesales", operationallyOpen: false }], quality: [{ id: "q1", employeeName: "TEST Telesales", evidence: "TEST review evidence", result: "needs_improvement", observation: "TEST observation", at: "2026-08-29T11:00:00Z" }], coaching: [{ id: "c1", employeeName: "TEST Telesales", topic: "TEST coaching", evidence: "proof", agreedAction: "TEST agreed action", dueAt: "2026-08-30T09:00:00Z", at: "2026-08-29T12:00:00Z", status: "open" }] };
...
async function fixture(page: any, options: { responses?: FixtureResponse[]; actionBody?: unknown } = {}) {
  ...
  const actions: Array<{ path: string; body: any }> = [];
  ...
  await page.route("**/api/**", async (route: any) => {
    ...
    if (method === "POST" && path.startsWith("/api/supervisor/")) { actions.push({ path, body: route.request().postDataJSON() }); return send(options.actionBody ?? { id: "saved", operationallyOpen: true }); }
    return send({}, 404);
  });
  ...
  return { workspaceRequests: () => workspaceRequests, actions };
}
...
test("Supervisor acknowledge and reassignment controls persist canonical action kinds", async ({ page }) => { const control=await fixture(page);await page.getByRole("button",{name:/TEST open exception/}).click();await page.locator("textarea").fill("TEST acknowledgement evidence");await page.getByRole("button",{name:"إقرار الدليل"}).click();await expect.poll(()=>control.actions.length).toBe(1);expect(control.actions[0].body).toMatchObject({kind:"acknowledge",evidence:"TEST acknowledgement evidence",version:1});await page.locator("textarea").fill("TEST reassignment evidence");await page.getByLabel("إعادة إسناد ضمن الفريق").selectOption("tel-1");await page.getByRole("button",{name:"حفظ إعادة الإسناد"}).click();await expect.poll(()=>control.actions.length).toBe(2);expect(control.actions[1].body).toMatchObject({kind:"reassign",ownerEmployeeId:"tel-1",evidence:"TEST reassignment evidence",version:1});expect(control.actions[1].body.followUpAt).toBeTruthy();});
...
test("Team and Activity navigation render distinct Supervisor surfaces", async ({ page }) => {
  await fixture(page);
  await page.getByRole("button", { name: "الفريق", exact: true }).click();
  await expect(page.locator("#supervisor-team")).toBeVisible();
  await expect(page.locator("#supervisor-activity")).toHaveCount(0);
  await expect(page.getByText("حجم الطابور والإنجاز الفعلي لكل عضو دون خلطه بسجل الجودة.")).toBeVisible();
  await page.getByRole("button", { name: "النشاط", exact: true }).click();
  await expect(page.locator("#supervisor-activity")).toBeVisible();
  await expect(page.locator("#supervisor-team")).toHaveCount(0);
  await expect(page.getByText("تقويم النشاط الإشرافي")).toBeVisible();
});

test("Supervisor Activity calendar opens canonical evidence for the selected day", async ({ page }) => {
  await fixture(page);
  await page.getByRole("button", { name: "النشاط", exact: true }).click();
  const day=page.locator('.activity-calendar__day[aria-label*="5 أدلة محفوظة"]');
  await day.click();
  await expect(page.locator(".activity-calendar__details")).toContainText("TEST open exception");
  await expect(page.locator(".activity-calendar__details")).toContainText("TEST review evidence");
});
