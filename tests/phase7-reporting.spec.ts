...
test("Manager reporting has real loading, retryable error, no-target and zero-evidence states in RTL mobile", async ({ page }) => {
  ...
  await expect(page.locator(".ds-state--loading").last()).toBeVisible(); await expect(page.getByText("TEST delayed")).toBeVisible(); await expect(page.getByText(/لا يوجد هدف TEST_DEMO/)).toBeVisible(); await expect(page.locator("#manager-reports").getByText(/لا توجد أدلة/)).toBeVisible();
