...
const ORDER_STEPS = ["مسجلة", "مراجعة الحسابات", "معتمدة", "التجهيز", "تجهيز التسليم", "تم التسليم"];
...
const SEED_CUSTOMERS = [
  {
    ...
    orders: [{ id: "o1", time: "اليوم 09:00", items: "منظف أرضيات 5 لتر × 12، صابون سائل 1 لتر × 24", value: "6,800 ج.م", currentStep: 3, responsible: "المخزن الرئيسي", next: "تجهيز الطلبية للتسليم غدًا", exception: null }],
    complaints: [],
  ...
  },
  {
    ...
    orders: [{ id: "o2", time: "منذ 5 أيام", items: "طقم فحمات فرامل × 4", value: "2,300 ج.م", currentStep: 4, responsible: "قسم التوزيع", next: "تسريع الشحنة وتأكيد موعد جديد", exception: "تأخر عن الموعد المتفق عليه 5 أيام" }],
...
function LifecycleSteps({ steps, currentStep, exception }) {
  return (
        ...
        <LedgerRegister open={open === "orders"} onToggle={() => toggle("orders")} mark={cust.orders.some(o => o.exception) ? "urgent" : "normal"} count={cust.orders.length} label="طلبات مسجلة" sub={cust.orders.some(o => o.exception) ? "يوجد استثناء يحتاج متابعة" : cust.orders.length ? "قيد التنفيذ" : "لا توجد طلبات"}>
          {cust.orders.length ? cust.orders.map(o => (
            <div className="order-detail" key={o.id}>
              ...
              <LifecycleSteps steps={ORDER_STEPS} currentStep={o.currentStep} exception={o.exception} />
