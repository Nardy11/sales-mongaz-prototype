...
const OUTCOME_TYPES = [
  { id: "done", label: "تم الاتفاق / التنفيذ", helper: "طلبية، تحصيل كامل، أو موافقة العميل" },
  { id: "promise", label: "وعد جديد", helper: "العميل التزم بموعد أو مبلغ لاحق" },
  { id: "no_contact", label: "لم يتم الوصول للعميل", helper: "لا رد، مغلق، أو تأجيل من العميل" },
  { id: "issue", label: "ظهرت مشكلة", helper: "شكوى أو عائق يحتاج متابعة" },
];
...
function deriveNextCommitment(item, outcomeType, amount, reason, escalate, preset) {
  const timing = TIMING_PRESETS.find(t => t.id === preset) || TIMING_PRESETS[0];
  let kind = item.kind;
  let next = "";
  if (outcomeType === "done") {
    kind = item.kind.includes("زيارة") ? "متابعة تسليم" : "متابعة تأكيد";
    next = "التأكد من وصول الطلبية للعميل";
  } else if (outcomeType === "promise") {
    kind = item.kind;
    next = amount ? `متابعة تحصيل ${amount} حسب الوعد الجديد` : "متابعة تحصيل الوعد الجديد";
  } else if (outcomeType === "no_contact") {
    kind = item.kind;
    next = `إعادة محاولة — ${reason || "لم يُحدَّد السبب"}`;
  } else {
    kind = escalate ? "متابعة تصعيد" : "متابعة حل مشكلة";
    next = escalate ? "متابعة تصعيد الشكوى مع المشرف" : "متابعة حل المشكلة مع العميل";
  }
  return {
    ...
    due: timing.due,
    next,
    amount: outcomeType === "promise" ? (amount || null) : null,
    phone: item.phone,
  };
}
...
function OutcomeCapture({ item, onSave, onCancel }) {
  ...
  const needsAmount = outcomeType === "promise";
  const needsReason = outcomeType === "no_contact";
  const needsNote = outcomeType === "issue";
  const canSave = started && !!outcomeType && (!needsReason || !!reason);

  const preview = outcomeType
    ? deriveNextCommitment(item, outcomeType, amount, reason, escalate, preset)
    : null;
  ...
  return (
    <div className="outcome">
      ...
      {started && (
        <div className="outcome-section">
          <div className="section-label">ما الذي حدث؟</div>
          <div className="option-list">
            {OUTCOME_TYPES.map(o => (
              <button
                key={o.id}
                className="option-row"
                onClick={() => setOutcomeType(o.id)}
              >
      ...
      {needsAmount && (
        <div className="outcome-section">
          <div className="section-label">قيمة الوعد</div>
          <input
            className="ledger-input nums"
            inputMode="numeric"
            placeholder="مثال: 3,000 ج.م"
            value={amount}
        ...
        <div className="outcome-section">
          ...
          <div className="chip-row">
            {TIMING_PRESETS.map(t => (
              <button
                key={t.id}
                className={"reason-chip" + (preset === t.id ? " active" : "")}
          ...
          {preview && (
            <div className="next-preview">
              ...
              <div className="next-preview-body">
                <div className="rail-kind">{preview.kind}</div>
                <div className="rail-next"><ArrowUpRight size={13} /><span>{preview.next}</span></div>
                <div className="rail-due">{preview.due}</div>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="outcome-actions">
        ...
        <button className="btn-primary" disabled={!canSave} onClick={() => onSave(preview)}>
          حفظ النتيجة
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  function saveOutcome(nextCommitment) {
    setStatusOverride(s => ({ ...s, [selected.id]: "completed" }));
    if (nextCommitment) {
      setInjected(s => ({ ...s, [roleId]: [...s[roleId], nextCommitment] }));
    }
    setSelected(null);
    setSheetMode("view");
    setToast(nextCommitment ? `تم تسجيل النتيجة — أُضيف التزام جديد: ${nextCommitment.due}` : "تم تسجيل النتيجة");
    window.clearTimeout(window.__toastT);
          ...
          <div className="sheet-overlay" onClick={closeSheet}>
            <div className="sheet" onClick={e => e.stopPropagation()}>
              ...
              {sheetMode === "view" ? (
                <>
                  ...
                  <div className="sheet-actions">
                    ...
                    {selected.status !== "completed" ? (
                      <button className="btn-primary" onClick={() => setSheetMode("capture")}>
                ...
                <div className="sheet-scroll">
                  <OutcomeCapture item={selected} onSave={saveOutcome} onCancel={() => setSheetMode("view")} />
                </div>
              )}
            </div>
          </div>
