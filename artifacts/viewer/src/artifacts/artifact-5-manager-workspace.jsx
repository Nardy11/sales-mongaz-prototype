...
const SEED_EXCEPTIONS = [
  ...
  },
  {
    ...
    evidence: ["الطلب: طقم فحمات فرامل × 4 — 2,300 ج.م", "الحالة الحالية: تجهيز التسليم (خطوة 5 من 6)"],
    currentStep: 4, status: "open",
  },
  {
  ...
  return (
    <div className="exc-detail">
      ...
      {exc.type === "order_blocked" && (
        <LifecycleSteps steps={ORDER_STEPS} currentStep={exc.currentStep} exception={`${exc.why} — قيمة الطلب ${exc.orderValue}`} />
      )}
...
function ReportsTab({ exceptions, development, actionsTakenCount }) {
  ...
  const rows = [
    ...
    { id: "orders", severity: "normal", count: exceptions.filter(x => x.type === "order_blocked").length, label: "طلبات معطَّلة", sub: "بانتظار أو قيد المعالجة", items: exceptions.filter(x => x.type === "order_blocked").map(x => `${x.customer} — ${x.why}`) },
