...
import { accountProfile, getSession, login, logout, searchCustomers, type SessionIdentity } from "../lib/api";
...
function RoleWorkspace({session,onLogout}:{session:SessionIdentity;onLogout:()=>void}) {
 ...
 return <div className="production-app-shell">
   <header className="production-appbar">
     ...
     <div className="artifact-appbar-actions"><button type="button" className={utility==="search"?"active":""} aria-label="بحث العملاء" aria-expanded={utility==="search"} onClick={()=>{keyboard.hide();setUtility(utility==="search"?null:"search");setShowStateGuide(false);}}><MagnifyingGlassIcon/></button><button type="button" className={utility==="notifications"?"active":""} aria-label="الإشعارات" aria-expanded={utility==="notifications"} onClick={()=>{keyboard.hide();setUtility(utility==="notifications"?null:"notifications");setShowStateGuide(false);}}><BellIcon/></button><button type="button" className={showStateGuide?"active":""} aria-label="دليل حالات الواجهة" aria-expanded={showStateGuide} onClick={()=>{keyboard.hide();setUtility(null);setShowStateGuide(true);}}><BookmarkIcon/></button></div>
   ...
   <MobileScroll className="production-screen"><main className="production-content">{showProfile?<ProfilePanel session={session} onClose={()=>setShowProfile(false)} onLogout={endSession}/>:customerId?<CustomerOperatingFile id={customerId} onBack={()=>setCustomerId(null)} session={session}/>:r==="sales_representative"?(tab==="customers"?<CustomerRegister onOpen={openCustomer}/>:<RepresentativeWorkspace session={session} activeTab={tab as "day"|"mine"|"activity"}/>):r==="telesales_employee"?<TelesalesWorkspace session={session} onCustomer={openCustomer} activeTab={tab as FrontlineTab}/>:r==="telesales_supervisor"?<SupervisorWorkspace session={session} activeTab={tab as "day"|"team"|"queues"|"activity"}/>:r==="sales_manager"?(tab==="customers"?<CustomerRegister onOpen={openCustomer}/>:<ManagerWorkspace session={session} activeTab={tab as "review"|"priorities"|"reports"}/>):<StatePanel kind="empty" title="مسار الدور مهيأ" detail="لا توجد وظائف تشغيلية لهذا الدور."/>}</main></MobileScroll>
   {utility&&<section className="artifact-utility-panel" role="dialog" aria-modal="true" aria-label={utility==="search"?"بحث العملاء":"الإشعارات"}><header><div><small>{utility==="search"?"بحث تشغيلي":"مركز التنبيهات"}</small><strong>{utility==="search"?"ابحث في العملاء المصرح بهم":"الإشعارات"}</strong></div><button type="button" onClick={()=>{keyboard.hide();setUtility(null);}} aria-label="إغلاق">×</button></header>{utility==="search"?<div className="artifact-utility-search"><label>اسم العميل أو الكود<KeyboardInput autoFocus value={searchTerm} onChange={event=>setSearchTerm(event.target.value)} placeholder="اكتب حرفين على الأقل"/></label>{searchTerm.trim().length<2?<StatePanel kind="empty" title="ابدأ البحث" detail="اكتب حرفين على الأقل لعرض العملاء داخل نطاقك."/>:search.isLoading?<StatePanel kind="loading" title="جارٍ البحث" detail="يتم جلب العملاء المصرح بهم من الخادم."/>:search.isError?<StatePanel kind="error" title="تعذر البحث" detail="أعد المحاولة لجلب نتائج العملاء." retry={()=>search.refetch()}/>:search.data?.length?<div className="artifact-search-results">{search.data.map(customer=><button type="button" key={customer.id} onClick={()=>openCustomer(customer.id)}><span><strong>{customer.name}</strong><small>{customer.customerCode??"بدون كود"} · {customerClassificationLabel(customer.classification)}</small></span><i>{customer.openCommitments} مفتوحة</i></button>)}</div>:<StatePanel kind="empty" title="لا توجد نتائج" detail="لا يوجد عميل مطابق داخل نطاق الوصول المصرح به."/>}</div>:<div className="artifact-notification-empty"><BellIcon/><StatePanel kind="empty" title="لا توجد إشعارات جديدة" detail="لا توجد إشعارات تشغيلية متاحة لهذا الحساب الآن."/></div>}</section>}
